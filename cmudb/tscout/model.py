...
class BPFVariable:
    ...
    def alignment_string(self):
        return (
            " __attribute__ ((aligned ({})))".format(self.alignment)
    ...
    def serialize(self, output_event):
        ...
        if self.c_type == clang.cindex.TypeKind.FLOAT:
            float_val = struct.unpack(
                "f",
                getattr(output_event, self.name).to_bytes(4, byteorder=sys.byteorder),
            )[0]
            float_val = round(float_val, FLOAT_DOUBLE_NDIGITS)
            return str(float_val)
        elif self.c_type == clang.cindex.TypeKind.DOUBLE:
            double_val = struct.unpack(
                "d",
                getattr(output_event, self.name).to_bytes(8, byteorder=sys.byteorder),
            )[0]
...
OU_METRICS = (
    BPFVariable(name="start_time", c_type=clang.cindex.TypeKind.ULONG, alignment=8),
    BPFVariable(name="end_time", c_type=clang.cindex.TypeKind.ULONG),
    BPFVariable(name="cpu_cycles", c_type=clang.cindex.TypeKind.ULONG),
    BPFVariable(name="instructions", c_type=clang.cindex.TypeKind.ULONG),
    BPFVariable(name="cache_references", c_type=clang.cindex.TypeKind.ULONG),
    BPFVariable(name="cache_misses", c_type=clang.cindex.TypeKind.ULONG),
    BPFVariable(name="ref_cpu_cycles", c_type=clang.cindex.TypeKind.ULONG),
    BPFVariable(name="network_bytes_read", c_type=clang.cindex.TypeKind.ULONG),
    BPFVariable(name="network_bytes_written", c_type=clang.cindex.TypeKind.ULONG),
    BPFVariable(name="disk_bytes_read", c_type=clang.cindex.TypeKind.ULONG),
    BPFVariable(name="disk_bytes_written", c_type=clang.cindex.TypeKind.ULONG),
    BPFVariable(name="memory_bytes", c_type=clang.cindex.TypeKind.ULONG),
    BPFVariable(name="elapsed_us", c_type=clang.cindex.TypeKind.ULONG),
    BPFVariable(name="pid", c_type=clang.cindex.TypeKind.UINT),
    BPFVariable(name="cpu_id", c_type=clang.cindex.TypeKind.UCHAR),
)
    ...
    def features_struct(self) -> str:
        ...
        for feature in self.features_list:
            if feature.readarg_p:
                ...
                assert (
                    len(feature.bpf_tuple) >= 1
                ), "We should have some fields in this struct."
                ...
                for column in feature.bpf_tuple:
                    struct_def = struct_def + (
                        "{} {}{};\n".format(
                            CLANG_TO_BPF[column.c_type],
                            column.name,
                            column.alignment_string(),
                        )
                    )
            else:
                ...
                assert (
                    len(feature.bpf_tuple) == 1
                ), "How can something not using readarg_p have multiple fields?"
                struct_def = struct_def + (
                    "{} {};\n".format(
                        CLANG_TO_BPF[feature.bpf_tuple[0].c_type],
                        feature.bpf_tuple[0].name,
                    )
                )
            ...
            for feature in features:
                ...
                for i, field in enumerate(nodes.field_map[feature.name]):
                    try:
                        ...
                        logger.critical(
                            "No mapping from Clang to BPF for type {} for field {} in the struct {}.".format(
                                e, field.name, feature.name
                            )
                        )
                        exit()
                new_feature = Feature(
                    feature.name, bpf_tuple=bpf_fields, readarg_p=True
                )
